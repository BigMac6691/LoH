...
export class MapModel {
    ...
    for (let i = 0; i < numStars; i++) {
      ...
      while (attempts < 50 && !starPlaced) {
        ...
        if (!tooClose) {
          ...
          const star = new Star({
            id: this.nextStarId++,
            name: null, // Will be generated by generateName()
            x, y, z,
  ...
  /**
   * Get a star by its ID using the fast lookup map
   * @param {string|number} starId - Star ID to look up
   * @returns {Star|null} Star object or null if not found
   */
  getStarById(starId) {
    return this.starLookup.get(starId) || null;
  }
...
export function generateMap({ seed, mapSize, densityMin, densityMax }) {
  ...
  // Convert to the expected interface using the new data-centric pattern
  const stars = model.stars.map(star => ({
    id: star.getId(),
    name: star.getName(),
    x: star.getPosition().x,
    y: star.getPosition().y,
    z: star.getPosition().z,
    sectorX: star.getSector().col,
    sectorY: star.getSector().row
  }));
  
  const edges = model.wormholes.map(wormhole => ({
    id: `edge_${wormhole.star1.getId()}_${wormhole.star2.getId()}`,
    aStarId: wormhole.star1.getId(),
    bStarId: wormhole.star2.getId()
  }));
