...
export class SplashScreen
{
   ...
   showLoginForm(showRegisterLink = true)
   {
      ...
      if (registerForm)
      {
         ...
         setTimeout(() => { registerForm.remove(); }, 500);
      }
   ...
   async handleLogin()
   {
      ...
      {
         this.showError( 'Password is required. If you forgot your password, please use password recovery.', {recover: true, email: email});
         return;
      }
   ...
   handleLoginFailure(data, email)
   {
      ...
      if (data.error === 'PASSWORD_REQUIRED')
         this.showError(data.message || 'Password is required. If you forgot your password, please use password recovery.', {recover: true, email: email});
   ...
   async handlePasswordReset()
   {
      ...
      try
      {
         const data = await RB.fetchPostUnauthenticated('/api/auth/reset-password', {token, newPassword});
   ...
   async handleRegistrationSubmit()
   {
      ...
      try
      {
         ...
         const data = await RB.fetchPostUnauthenticated('/api/auth/register', {email, password, displayName});
