...
export class GameRouter
{
   ...
   setupRoutes()
   {
      ...
      // POST /api/games/:gameId/startGame - Start a game (begin initialization)
      this.router.post('/:gameId/startGame', authenticate, requireRole(['sponsor', 'admin', 'owner']), this.startGame.bind(this));
   ...
   async createGame(req, res)
   {
      try
      {
         const { ownerId, seed, mapSize, densityMin, densityMax, title, description, maxPlayers, status, params } = req.body;

         // Debug logging
         console.log('Received request body:', req.body);
         console.log('Extracted values:', { ownerId, seed, mapSize, densityMin, densityMax, title, description, maxPlayers, status, params });

         // Validate required parameters
         if (!seed || !mapSize || densityMin === undefined || densityMax === undefined || !title || !description || !status || !ownerId)
            return res.status(400).json({success: false, error: 'Missing required parameters: seed, mapSize, densityMin, densityMax, title, description, status, ownerId'});

         // Validate maxPlayers if provided
         if (maxPlayers !== undefined)
         {
            if (maxPlayers < 1 || maxPlayers > mapSize * 2)
               return res.status(400).json({success: false, error: `maxPlayers must be between 1 and ${mapSize * 2} (mapSize * 2)`});
         }
      ...
      catch (error)
      {
         ...
         res.status(500).json({success: false, error: 'Failed to start game', details: error.message});
      }
   }
   ...
   async listGames(req, res)
   {
      ...
      catch (error)
      {
         ...
         res.status(500).json({success: false, error: 'Failed to get games list', details: error.message});
      }
   }
   ...
   async listGamesUnified(req, res)
   {
      try
      {
         ...
         if (!['playing', 'available', 'manage', 'all'].includes(filter))
            return res.status(400).json({success: false, error: 'Invalid filter. Must be one of: playing, available, manage, all'});

         // Validate manage filter requires appropriate role
         if (filter === 'manage' && !['sponsor', 'admin', 'owner'].includes(userRole))
            return res.status(403).json({success: false, error: 'Insufficient permissions. Manage filter requires sponsor, admin, or owner role.'});
         ...
         if (filter === 'playing') // User is a player (must have matching game_player record)
            whereConditions.push(`gp_user.id IS NOT NULL`); // User is a player (must have matching game_player record)
         else if (filter === 'available') // User is not a player, status is lobby, not full
         {
      ...
      catch (error)
      {
         ...
         res.status(500).json({success: false, error: 'Failed to list games', details: error.message});
      }
   }
   ...
   async joinGame(req, res)
   {
      try
      {
         ...
         if (req.user.role === 'visitor')
            return res.status(403).json({success: false, error: 'FORBIDDEN', message: 'Email verification required to join games'});
         ...
         const { rows: existingCountryRows } = await pool.query(
            `SELECT id FROM game_player WHERE game_id = $1 AND country_name = $2`,
            [gameId, trimmedCountryName]
         );

         if (existingCountryRows.length > 0)
            return res.status(400).json({success: false, error: 'Country name is already taken in this game'});

         // Get user info for default name if not provided
         if (!name || !colorHex)
         {
            const { rows: userRows } = await pool.query(`SELECT display_name FROM app_user WHERE id = $1`, [userId]);
            ...
            const { rows: existingColors } = await pool.query(`SELECT color_hex FROM game_player WHERE game_id = $1`, [gameId]);
            const usedColors = existingColors.map(r => r.color_hex);
            const availableColor = colors.find(c => !usedColors.includes(c)) || colors[0];
            const playerColorHex = colorHex || availableColor;

            // Add player to game
            const { addPlayer } = await import('../repos/playersRepo.js');
            const result = await addPlayer({ gameId, userId, name: playerName, colorHex: playerColorHex, countryName: trimmedCountryName, meta: {} });
            return res.json({success: true, message: 'Successfully joined game', gameId, player: {id: result.id, name: result.name, colorHex: result.color_hex}});
         }

         // Add player with provided name and color
         const result = await addPlayerToGame({ gameId, userId, name, colorHex, countryName: countryName ? countryName.trim() : null, meta: {} });
         res.json({success: true, message: 'Successfully joined game', gameId, player: {id: result.id, name: result.name, colorHex: result.color_hex}});
      }
      catch (error)
      {
         ...
         if (error.code === '23505')
            return res.status(400).json({success: false, error: 'Name or color already taken in this game'});

         res.status(500).json({success: false, error: 'Failed to join game', details: error.message});
      }
   }
   ...
   async getGame(req, res)
   {
      try
      {
         ...
         res.json({success: true, game: gameRows[0]});
      }
      catch (error)
      {
   ...
   async getGameState(req, res)
   {
      try
      {
         const { gameId } = req.params;
         if (!gameId)
            return res.status(400).json({success: false, error: 'gameId parameter is required'});
         ...
         if (currentPlayerRows.length === 1)
            ...
            return res.status(400).json({success: false, error: 'Unable to determine current player id'});
         ...
         const orders = currentTurn ? await getOrdersForTurn(gameId, currentTurn.id) : [];
         const events = currentTurn ? await getAllTurnEvents(gameId, currentTurn.id) : [];
         ...
         res.json({success: true, gameId, currentPlayerId, stars, wormholes, players, gameInfo, turn: currentTurn, starStates, ships, orders, events, counts});
      }
      catch (error)
      {
   ...
   async getCurrentGame(req, res)
   {
      try
      {
         ...
         const { rows: gamesRows } = await pool.query(`SELECT * FROM game ORDER BY created_at DESC LIMIT 1`);
         ...
         res.json({success: true, game, turn, players});

      }
      catch (error)
      {
         console.error('Error getting current game:', error);
         res.status(500).json({success: false, error: 'Failed to get current game', details: error.message});
      }
   }
   ...
   async addPlayer(req, res)
   {
      try
      {
         ...
         if (!gameId || !name || !colorHex)
            return res.status(400).json({success: false, error: 'Missing required parameters: gameId, name, colorHex'});
         ...
         const result = await addPlayerToGame({ gameId, userId, name, colorHex, countryName, meta: meta || {} });

         res.json({success: true, id: result.id, gameId: result.game_id, name: result.name, colorHex: result.color_hex, countryName: result.country_name});
      }
      catch (error)
      {
         console.error('Error adding player:', error);
         res.status(500).json({success: false, error: 'Failed to add player', details: error.message});
      }
   }
   ...
   async addAIPlayer(req, res)
   {
      try
      {
         ...
         if (existingPlayerRows.length > 0)
            return res.status(400).json({success: false, error: 'Player name is already taken in this game'});
         ...
         if (!aiRegistry.hasAI(aiName))
            return res.status(400).json({success: false, error: `AI '${aiName}' is not available`});
         ...
         if (schema)
         {
            ...
            if (invalidKeys.length > 0)
               return res.status(400).json({ success: false, error: `Invalid configuration keys: ${invalidKeys.join(', ')}`});
         }
         ...
         const meta = {main_ai: aiName, ai_config: aiConfig || {}};

         // Add AI player to game, players have no user_id
         const { addPlayer: addPlayerToGame } = await import('../repos/playersRepo.js');
      ...
      catch (error)
      {
         ...
         res.status(500).json({success: false, error: 'Failed to add AI player', details: error.message});
      }
   }
   ...
   async generateMap(req, res)
   {
      try
      {
         ...
         // Validate required parameters
         if (!gameId)
            return res.status(400).json({error: 'Missing required parameter: gameId'});
   ...
   async placePlayers(req, res)
   {
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to place players', details: error.message});
      }
   }
   ...
   async getOpenTurn(req, res)
   {
      try
      {
         ...
         if (turn)
            ...
            res.json({success: true, turn: null});
      }
      catch (error)
      {
         console.error('Error getting open turn:', error);
         res.status(500).json({error: 'Failed to get open turn', details: error.message});
      }
   }
   ...
   async createTurn(req, res)
   {
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to create turn', details: error.message});
      }
   }
   ...
   async getTurns(req, res)
   {
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to get turns', details: error.message});
      }
   }
   ...
   async getManageGamePlayers(req, res)
   {
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to get game players', details: error.message});
      }
   }

   /**
    * POST /api/games/:gameId/startGame
    * Start a game (begin initialization process)
    */
   ...
   async updateGameStatus(req, res)
   {
      ...
      const { gameId } = req.params;
      const { status, statusReason, version } = req.body;
      const response = {gameId, game: null}
   ...
   async endPlayerTurn(req, res)
   {
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to end player turn', details: error.message});
      }
   }
   ...
   async updatePlayerStatus(req, res)
   {
      try
      {
         ...
         const { rows: playerRows } = await pool.query( `SELECT * FROM game_player WHERE id = $1 AND game_id = $2`, [playerId, gameId]);
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to update player status', details: error.message});
      }
   }
   ...
   async updatePlayerMeta(req, res)
   {
      try
      {
         ...
         if (!gameId || !playerId || !meta)
            return res.status(400).json({error: 'Game ID, player ID, and meta are required'});
         ...
         if (typeof meta === 'string')
         {
            ...
            return res.status(400).json({error: 'Meta must be a valid JSON object or string'});
      ...
      catch (error)
      {
         ...
         res.status(500).json({error: 'Failed to update player meta', details: error.message});
      }
   }
