 ...
 */
export async function getNextSequenceNumber({ gameId, turnId }) {
 ...
 */
export async function logEvent({ gameId, turnId, kind, details }) {
  const client = await pool.connect();
  ...
  try {
    ...
    const seq = await getNextSequenceNumber({ gameId, turnId });
 ...
 */
export async function findEvents({ gameId, turnId, jsonFilter }) {
