...
export class WebSocketService
{
      ...
      this.io.on('connection', (socket) =>
      {
         ...
         socket.on('game:join', async (data) =>
         {
            ...
            try
            {
               ...
               if (!gameId)
                  return socket.emit('error', { message: 'gameId required' });

               // Verify user is a player in this game
               const {pool } = await import('../db/pool.js');
               const { rows } = await pool.query(`SELECT id FROM game_player WHERE game_id = $1 AND user_id = $2 AND type = 'player'`, [gameId, socket.data.userId]);
               if (rows.length === 0)
                  return socket.emit('error', { message: 'Not a player in this game' });

               // Register connection
               this.gameConnections.set(socket.id, { userId: socket.data.userId, gameId, playerId: playerId || rows[0].id });
         ...
         socket.on('game:leave', () =>
         {
            if (this.gameConnections.has(socket.id))
            {
               ...
               this.gameConnections.delete(socket.id);
            }
   ...
   {
      if (!this.isReady())
         return console.warn('🔌 WebSocketService: Cannot notify - Socket.IO not initialized');

      // Find all connections for this game
      const gameConnections = [];
      for (const [socketId, connection] of this.gameConnections.entries())
         if (connection.gameId === gameId)
            gameConnections.push(socketId);

      if (gameConnections.length === 0)
         return console.log(`🔌 WebSocketService: No active connections for game ${gameId}`);
      ...
      try
      {
         ...
         if (previousTurnId)
         {
            ...
            const { rows: turnRows } = await pool.query(`SELECT id, number FROM game_turn WHERE id = $1 AND game_id = $2`, [previousTurnId, gameId]);
         ...
         const refreshData = { success: true, type: 'game:turnComplete', gameId, turn: turn || null, state: starStates || [], ships: ships || [], orders: orders || [], events: events || [], ...updateData };
      ...
      catch (error)
      {
         ...
         gameConnections.forEach(socketId =>
         {
            ...
            if (socket)
               socket.emit('game:turnComplete', { success: true, type: 'game:turnComplete', gameId, turn: null, state: [], ships: [], orders: [], events: [], ...updateData });
         });
      }
   }
   ...
   {
      let count = 0;
      for (const connection of this.gameConnections.values())
         if (connection.gameId === gameId)
            count++;
    ...
    */
   getAllGameConnections()
   {
      return Array.from(this.gameConnections.entries()).map(([socketId, connection]) => ({ socketId, ...connection }));
   }
